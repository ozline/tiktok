...
func GetComments(ctx context.Context, videoId int64) (comments *[]redis.Z, err error) {
	rComments, err := Rdb.ZRevRangeWithScores(ctx, strconv.FormatInt(videoId, 10), 0, -1).Result()
	if err != nil {
		klog.Infof("Error: %v\n", err)
		return nil, err
	}
...
func AddComment(ctx context.Context, videoId int64, comment *Comment, date float64) (err error) {
	...
	err = Rdb.ZAdd(ctx, strconv.FormatInt(videoId, 10), redis.Z{Score: date, Member: data}).Err()
	if err != nil {
		klog.Infof("Error: %v\n", err)
	} else {
...
func AddComments(ctx context.Context, videoId int64, comments *[]Comment, date *[]float64) (err error) {
	...
	a, err := Rdb.ZAdd(ctx, strconv.FormatInt(videoId, 10), zComments...).Result()
	fmt.Println(a)
	if err != nil {
		klog.Infof("Error: %v\n", err)
		return err
	}
	err = Rdb.Expire(ctx, strconv.FormatInt(videoId, 10), 60*time.Minute).Err()
	if err != nil {
		klog.Infof("Error: %v\n", err)
	} else {
...
func DeleteComment(ctx context.Context, videoId int64, comment *Comment) (err error) {
	...
	err = Rdb.ZRem(ctx, strconv.FormatInt(videoId, 10), data).Err()
	if err != nil {
		klog.Infof("Error: %v\n", err)
	} else {
...
func CountComments(ctx context.Context, videoId int64) (count int64, err error) {
	count, err = Rdb.ZCard(ctx, strconv.FormatInt(videoId, 10)).Result()
	if err != nil {
		klog.Infof("Error: %v\n", err)
	} else {
...
func IsExistComment(ctx context.Context, videoId int64) (exist int64, err error) {
	fmt.Println()
	exist, err = Rdb.Exists(ctx, strconv.FormatInt(videoId, 10)).Result()
	if err != nil {
		klog.Infof("Error: %v\n", err)
	} else {
